...
__device__ double wrapped_fitness_function(double x[], int number_of_variables,
                                           double* xopt, double* M, double* b, double fopt, double asymmetric_beta)
{
    transform_vars_shift(x, number_of_variables, xopt);
    transform_vars_affine(x, number_of_variables, M, b);
    transform_vars_asymmetric(x, number_of_variables, asymmetric_beta);
    transform_vars_affine(x, number_of_variables, M, b);
...
extern "C" {
                                      ...
                                      double* xopt, double* M, double* b, double fopt, double asymmetric_beta)
    {
        ...
        if(wrapped_fitness_function(tempParticle1, dimensionsCount, xopt, M, b, fopt, asymmetric_beta) <
           wrapped_fitness_function(tempParticle2, dimensionsCount, xopt, M, b, fopt, asymmetric_beta))
        {
