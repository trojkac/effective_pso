...
\noindent \textbf{Słowa kluczowe.} Algorytm optymalizacyjny, Particle Swarm Optimization, CUDA
...
\begin{abstract}
In this paper, we present a distributed system engineering optimization function allows to run calculations on a cluster of several computers, the base engine optimization algorithm Particle Swarm Optimization. The program was written with particular emphasis on the optimization of continuous functions with the benchmark BBOB but allows the programmer to extend the functionality to any type of function. The algorithm was implemented in the form of libraries in .NET that allows adjustment to the hardware on which it runs, and the use of multiprocessing local machine, as well as launching distributed computing on multiple computers simultaneously using a WCF service. The library also allows for the use of parallel computing GPUs support CUDA technology, and provides a programming interface that gives the opportunity to implement their own version of the PSO algorithm, which portray preparing a sample implementation of the two versions of the algorithm. As a test of the present test results of our library using the platform COCO and optimization benchmark used during the workshop BBOB. Test results show a clear profit at the start of distributed computing on multiple cluster nodes at the same time a small profit, which we managed to get by using parallel computing on the graphics card.\\

\noindent \textbf{Keywords.} Optimization algorithm, Particle Swarm Optimization, CUDA
\end{abstract}
...
Przy wykorzystaniu obliczeń na GPU zaobserwowaliśmy średnio jedynie nieznaczną poprawę jakości. Pomimo faktu, że na rój na karcie graficznej był 16-krotnie liczniejszy niż na CPU, fakt, że implementował on jedynie najprostszą wersję PSO, nie wykorzystującą mechanizmu restartów cząstek, mógł odpowiadać za jego relatywnie niską skuteczność. Nieco bardziej zauważalna poprawa nastąpiła jedynie dla funkcji sferycznej oraz elipsoidalnej. 
