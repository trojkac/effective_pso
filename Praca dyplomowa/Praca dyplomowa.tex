...
Następujące informacje konfiguracyjne, zamknięte w klasie \emph{PsoParams}, wykorzystane są do uruchomienia algorytmu:  
...
\lstset{style=sharpc}
\begin{lstlisting}[frame=single]
public void UpdateVelocity(IState<double[], double[]> globalBest)
{
...
\lstset{style=sharpc}
\begin{lstlisting}[frame=single]
public void Transpose(IFitnessFunction<double[], double[]> function)
{
  double[] newLocation;
  ...
  {
    newLocation = ClampLocation(CurrentState.Location + Velocity);
  }
...
Funkcje celu są obiektami implementującymi interfejs IFitnessFunction. Udostępniają one liczbę przeprowadzonych ewaluacji funkcji, najlepszą ewaluację oraz informacje o wymiarze przestrzeni rozwiązań i wartości funkcji. Funkcje są tworzone przez obiekt FunctionsFactory na podstawie przekazanych do niej parametrów. W celu stworzenia odpowiedniej funkcji klient musi przekazać jej odpowiednią nazwę. Jest to szczególnie ważne w przypadku funkcji benchamrkowych, gdyż w nazwie zakodowany jest typ funkcji oraz jej odpowiednia instancja. Dodatkowo obiekt FunctionsFactory umożliwia zapisanie danej funkcji w cache'u. Konieczność zastosowania pamięci tymczasowej była wynikiem integracji systemu z biblioteką COCO, która wymagała użycia jednej instancji funkcji celu w całym systemie.
...
UpdateNodes - wysyła aktualne informacje o wszystkich zarejestrowanych węzłach klastra. Ten komunikat wysyłany jest po dołączeniu nowego węzła do sieci.

Register - dodaje węzeł wywołujący zdalnie tę usługę do listy znanych węzłów węzła, który wykonuje usługę.

Deregister - usuwa węzeł podany jako parametr metody (zawartość wiadomości) z listy znanych węzłów węzła ją wykonującego.
...
CalculationsFinished - wysyła informację o zakończeniu obliczeń i osiągniętym wyniku do węzła inicjującego obliczenia.

Przykładowy schemat komunikacji pomiędzy węzłami jest przedstawiony poniżej.
