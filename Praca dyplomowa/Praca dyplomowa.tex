...
\newcommand\MyIm[2]{%
  \captionof*{subfigure}{}
  \includegraphics[width=22em,height=20em]{#1}
...
\begin{figure}[H]
 ...
 \caption{Rysunek przedstawia kilka przykładowych topologii roju cząstek, gdzie linie łączą cząstki będące sąsiadami: (1) pełne sąsiedztwo, (2) pierścień, (3) topologia von Neumanna, (4) losowe sąsiedztwo. (Rysunek pochodzi z \cite{topologieRys})}
\end{figure}
...
Obliczenia są niezależne od połączeń pomiędzy węzłami klastra, dzięki czemu po awarii jednego z nich obliczenia nie są przerywane, a topologia sąsiedztwa rojów algorytmu PSO jest dostosowywana do dostępnych w danym momencie węzłów. Niestety awaria węzła głównego, czyli tego, który rozpoczynał obliczenia, mimo, że ich nie przerywa, to jednak uniemożliwia późniejsze zebranie wyników, jako że jedynie na tym węźle są one gromadzone.
...
Zatem dopóki nie zostanie spełniony pewien warunek zakończenia, dla każdej cząstki aktualizujemy jej pozycję, a następnie dla każdej cząstki aktualizujemy jej prędkość.
...
Biblioteka ta umożliwia wykorzystanie potencjału kart graficznych wspierających technologię CUDA z poziomu platformy .NET. Daje ona dostęp do informacji o dostępnym sprzęcie, ułatwia zarządzanie pamięcią karty graficznej oraz umożliwia wykonywanie kerneli CUDA z poziomu kodu C\#. 
...
Przygotowany przez nas system ma postać biblioteki .NET, jednakże dla wygody użytkownika oraz testera przygotowaliśmy aplikację pozwalającą na uruchomienie benchmarku BBOB. Szczegóły zostały opisane poniżej.

\section{CocoClusterApp}

CocoClusterApp jest aplikacją kliencką naszej biblioteki umożliwiającą uruchomienie benchmarku BBOB we wszystkich wspieranych przez naszą bibliotekę ustawieniach.

\subsection{Konfiguracja aplikacji}

\subsubsection{Klaster obliczeniowy}

Plik konfigurujący klaster obliczeniowy jest napisany w języku XML. Korzeniem dokumentu jest element o nazwie \textit{NodeParameters}. 
Dziećmi elementu głównego są elementy:
\begin{enumerate}
	\item \texttt{NrOfVCpu} - wartość liczbowa definiująca liczbę węzłów, które zostaną utworzone przez program. Gdy przypiszemy mu wartość -1 obliczenia zostaną uruchomione na liczbie wątków dopasowanej do procesora.
	\item \texttt{Ip} - adres interfejsu sieciowego maszyny, z którą będą mogły połączyć się pozostałe wątki klastra
	\item \texttt{Ports} - tablica integerów z numerami portów, na których nasłuchiwać będą usługi WCF
	\item \texttt{PeerAddress} - element zawierający adres węzła, do którego aplikacja powinna się podłączyć. Powinien zostać pominięty w przypadku, gdy użytkownik nie ma zamiaru łączyć się z innymi jednostkami
\end{enumerate}
...
\begin{enumerate}
	\item \texttt{Particles} - tablica definiująca liczbę oraz rodzaj cząstek użytych do obliczeń
	\item \texttt{GpuParameters} - definiujące parametry roju GPU. Dziećmi tego elementu są:
	\begin{enumerate}
		\item \texttt{UseGpu} - wartość logiczna decydująca o tym, czy używać GPU w obliczeniach
		\item \texttt{ParticlesCount} - liczba cząstek na GPU
		\item \texttt{Iterations} - maksymalna liczba iteracji do wykonania na GPU
	\end{enumerate}
	\item \texttt{PsoIterationsToRestart} - liczba iteracji algorytmu bez poprawy wyniku, która musi zostać wykonana zanim zostanie on zatrzymany
	\item \texttt{ParticleIterationsToRestart} - liczba iteracji algorytmu bez poprawy wyniku danej cząstki, po której cząstka zostanie zrestartowana
...
W przypadku awarii jednego lub większej liczby zwykłych węzłów obliczenia nie są przerywane, a klaster odtwarza topologię pierścienia. Jedynie utrata węzła głównego, rozpoczynającego obliczenia powoduje ich zakończenie wraz z zakończeniem przetwarzania obecnego problemu optymalizacyjnego przez pozostałe węzły, gdyż węzeł główny konieczny jest do zainicjowania obliczeń każdego kolejnego problemu oraz odpowiada za zbieranie wyników obliczeń z całego klastra.
...
Poniżej znajdują się wyniki i interpretacja testów naszej biblioteki przeprowadzonych przy użyciu platformy COCO oraz zestawu benchmarków BBOB.

\section{Plan testów}

Bazową konfiguracją, względem której oceniamy wyniki pozostałych testów, jest rój złożony ze standardowych cząstek uruchomiony na jednym węźle obliczeniowym niekorzystającym z GPU.\\
...
W celu sprawdzenia zysku, jaki daje rozproszenie obliczeń na więcej węzłów obliczeniowych, przeprowadzamy testy na dwóch klastrach - jednym złożonym z 2 identycznych węzłów, drugim składający się z 4 identycznych węzłów. Na każdym węźle znajduje się rój złożony z 40 standardowych cząstek; żaden z węzłów nie korzysta z pomocy GPU.
...
% * jak działają benchmarki (powtórka)
% * jakie wyniki przedstawiamy

Dla każdej z opisanych powyżej konfiguracji testowych przeprowadzamy taki sam zestaw testów, które, zgodnie z opisem benchmarku BBOB, składają się z 24 typów funkcji celu służących do zbudowania 15 konkretnych instancji funkcji, różniących się przesunięciem względem początku układu współrzędnych, obrotem, rozciągnięciem lub inną prostą transformacją nie wpływającą na istotę problemu - wyniki dla tych instancji są następnie uśredniane i prezentowane jako wyniki dla danej funkcji celu. Dodatkowym parametrem jest wymiar funkcji celu przyjmujący wartości ze zbioru \{2, 3, 5, 10, 20\} - dla każdego z nich przeprowadzamy pełen zestaw testów.

Poniżej przedstawiamy dwa rodzaje wykresów:

\begin{enumerate}

\item Dla każdej z konfiguracji testowych prezentujemy uśredniony po wszystkich funkcjach wykres zależności proporcji osiągniętych celów od liczby ewaluacji funkcji celu (a dokładniej od $log_{10}(l. ewal. / wymiar)$) dla każdego z badanych wymiarów funkcji, a obok niego również uśredniony dla wszystkich funkcji wykres przedstawiający proporcję osiągniętych celów (odległości od optimum) w zależności od liczby ewaluacji ($log_{10}(l. ewal. / wymiar)$), dla różnych progów dokładności ze zbioru \{$10^{1}$, $10^{-1}$, $10^{-4}$, $10^{-8}$\}.

\item Dla pojedynczego węzła oraz dla klastra obliczeniowego prezentujemy wykresy porównawcze, dla różnych rodzajów funkcji testowych. Na pierwszej grupie wykresów (obrazującej działanie pojedynczego węzła) znajduje się porównanie roju standardowego z rojem korzystającym z cząstek naładowanych oraz rojem korzystającym z pomocy GPU. Druga grupa wykresów przedstawia porównanie działania klastra dla 1, 2 i 4 węzłów obliczeniowych.
...
\newcommand{\addpic}[1]{\includegraphics[width=20em, height=16.5em]{#1}}
\newcolumntype{C}{>{\centering\arraybackslash}m{18em}}
\begin{table}\sffamily
\begin{tabular}{l*3{C}@{}}
\toprule
Id. & a & b \\ 
\midrule
1 & \addpic{wykresy/1node/ogolny_40st.pdf} & \addpic{wykresy/1node/okul_40st.pdf} \\ 
2 & \addpic{wykresy/1node/ogolny_20ch_20st.pdf} & \addpic{wykresy/1node/okul_20ch_20st.pdf} \\
3 & \addpic{wykresy/1node/ogolny_40st_gpu.pdf} & \addpic{wykresy/1node/okul_40st_gpu.pdf} \\ 
\bottomrule 
\end{tabular}
\caption{Wyniki dla różnych konfiguracji 1 węzła obliczeniowego. Od góry są to: 1. 40 cząstek standardowych 2. 20 cząstek standardowych i 20 cząstek naładowanych 3. 40 cząstek standardowych i 640 cząstek GPU.}
\end{table}
...
Id. & a & b \\ 
\midrule
1 & \addpic{wykresy/1node/comp_10D_noiselessall.pdf} & \addpic{wykresy/1node/comp_10D_separ.pdf} \\ 
1 & \addpic{wykresy/1node/comp_10D_lcond.pdf} & \addpic{wykresy/1node/comp_10D_hcond.pdf} \\
1 & \addpic{wykresy/1node/comp_10D_multi.pdf} & \addpic{wykresy/1node/comp_10D_mult2.pdf} \\ 
\bottomrule 
\end{tabular}
\caption{Porównanie wyników dla różnych konfiguracji 1 węzła obliczeniowego. 1a. Wszystkie funkcje 10D, 1b. Separable functions, 2a. Misc. moderate functions, 2b. Ill-conditioned functions, 3a. Multi-modal functions, 3b. Weak structure functions}
\end{table}
...
Id. & a & b \\ 
\midrule
1 & \addpic{wykresy/klaster/ogolny_1node.pdf} & \addpic{wykresy/klaster/okul_1node.pdf} \\ 
2 & \addpic{wykresy/klaster/ogolny_2node.pdf} & \addpic{wykresy/klaster/okul_2node.pdf} \\
3 & \addpic{wykresy/klaster/ogolny_4node.pdf} & \addpic{wykresy/klaster/okul_4node.pdf} \\ 
\bottomrule 
\end{tabular}
\caption{Wyniki dla różnych konfiguracji klastra obliczeniowego. Od góry są to: 1. 1 węzeł obliczeniowy 2. 2 węzły obliczeniowe 3. 4 węzły obliczeniowe.}
\end{table}
...
Id. & a & b \\ 
\midrule
1 & \addpic{wykresy/klaster/comp_10D_noiselessall.pdf} & \addpic{wykresy/klaster/comp_10D_separ.pdf} \\ 
1 & \addpic{wykresy/klaster/comp_10D_lcond.pdf} & \addpic{wykresy/klaster/comp_10D_hcond.pdf} \\
1 & \addpic{wykresy/klaster/comp_10D_multi.pdf} & \addpic{wykresy/klaster/comp_10D_mult2.pdf} \\ 
\bottomrule 
\end{tabular}
\caption{Porównanie wyników dla różnych konfiguracji klastra obliczeniowego. 1a. Wszystkie funkcje 10D, 1b. Separable functions, 2a. Misc. moderate functions, 2b. Ill-conditioned functions, 3a. Multi-modal functions, 3b. Weak structure functions}
\end{table} 
...
Porównanie wyników standardowego PSO z wynikami uzyskanymi po zastąpieniu połowy cząstek roju cząstkami naładowanymi wskazuje niestety na niewielką różnicę roju zhybrydyzowanego w stosunku do roju standardowego. Nieliczne odstępstwa od tej reguły, takie jak znacząco lepszy wynik dla 20-wymiarowej funkcji sferycznej (funkcja nr 1 z BBOB) oraz poprawa dla 10- i 20-wymiarowych funkcji Gallaghera (funkcje nr 21 i 22) nie wpływają na średni wynik porównania. Charged PSO zostało pierwotnie stworzone z myślą o zastosowaniu w problemie optymalizacji dynamicznej, zaś w badanych przez nas scenariuszach sprawdza się bardzo podobnie do standardowego PSO.
...
Przy wykorzystaniu obliczeń na GPU zaobserwowaliśmy średnio jedynie nieznaczną poprawę jakości. Pomimo tego, że rój na karcie graficznej był 16-krotnie liczniejszy niż na CPU, fakt że implementował on najprostszą wersję PSO, nie wykorzystującą mechanizmu restartów cząstek, mógł odpowiadać za jego relatywnie niską skuteczność, pomimo, że duża liczba cząstek powodowała wykonywanie między 2-, a 11-krotnie więcej ewaluacji dla każdej z funkcji testowych. Podobnie jak w przypadku hybrydyzacji algorytmu z algorytmem charged PSO, również wykorzystanie GPU przyniosło znaczącą poprawę jedynie dla niewielu wybranych funkcji (m.in. 10-wymiarowej funkcji elipsoidalnej (funkcja nr 2)), jednak nie przełożyło się to na znaczącą średnią poprawę rezultatów. 
...
% zauważyć, że im większy wymiar, tym dalej od best 2009

\subsection{Wnioski}

Obliczenia dla różnych konfiguracji testowych na pojedynczym węźle obliczeniowym wykazały, że zarówno wykorzystana przez nas hybrydyzacja algorytmu PSO z algorytmem charged PSO jak i wspomaganie obliczeń rojem cząstek na GPU nie przyniosło znaczącej poprawy wyników.

Wykresy przedstawiające porównanie wyników dla różnych grup funkcji wskazują co prawda, że wykorzystanie GPU przynosiło zazwyczaj pewną poprawę wyników (najlepiej widoczną dla 10-wymiarowych funkcji u numerach 1-5), nie można jednak mówić o dużym wpływie obliczeń na procesorze graficznym na wyniki. Podobne średnie wyniki dla wszystkich trzech konfiguracji odpowiadają podobnym wynikom dla większości funkcji testowych. Jedynie w nielicznych przypadkach pojawiały się znaczące różnice między poszczególnymi wersjami algorytmu, jednakże często nie udawało się ich powtórzyć przy zmianie liczby wymiarów, toteż nie sposób wyciągnąć na tej podstawie wniosków co do lepszego radzenia sobie konkretnej konfiguracji z daną funkcją czy grupą funkcji.\\

Zakładany postęp udało nam się osiągnąć dla obliczeń na klastrze, gdzie w wyraźny sposób większa liczba węzłów, a zatem większa moc obliczeniowa dawała lepsze wyniki testów. W niemal każdym przypadku większa liczba węzłów obliczeniowych skutkowała poprawą rezultatów (z ciekawym wyjątkiem 10- i 20-wymiarowej funkcji Gallaghera, gdzie klaster złożony z 2 węzłów zwracał lepszy wynik od klastra 4 węzłów), jednakże różna była skala poprawy. W niektórych przypadkach użycie klastra powodowało bardzo znaczącą różnicę, jak np. dla 20-wymiarowej funkcji elipsoidalnej, gdzie już 2 węzły powodowały osiągnięcie 10-krotnie większej liczby celów niż osiągał węzeł pojedynczy, w innych zaś, jak np. dla 10- i 20-wymiarowych funkcji wielomodalnych (funkcje o nr 15-19), różnica była bardzo słabo widoczna i praktycznie nieistotna.

\subsection{Poprawa wyników}

Brak wyraźnej poprawy wyników w przypadku zastosowania obliczeń równoległych na GPU jest zastanawiający w świetle przytoczonych we wstępie rezultatów wykorzystania procesorów graficznych do wspomagania algorytmu PSO, wskazujących na możliwość kilkunasto-, a nawet kilkudziesięciokrotnego przyspieszenia obliczeń przy ich pomocy. Podejrzewamy, że w przypadku naszego systemu wynika on z pewnych przyjętych przez nas uproszczeń przy implementacji algorytmu w technologii CUDA i łączeniu go z pozostałą częścią systemu. Po pierwsze, na GPU zaimplementowaliśmy jedynie najprostszą wersję algorytmu, nie korzystającą m.in. z restartów cząstek i rojów, które to znacząco poprawiły jakość naszego algorytmu w wersji na CPU. Dodatkowo na karcie graficznej nie przetestowaliśmy innych możliwych topologii roju, co również mogłoby wpłynąć na wynik. Innego rodzaju problemem okazała się komunikacja na poziomie algorytmu pomiędzy kartą graficzną a resztą systemu, która nie została przez nas zoptymalizowana.

Hybrydyzacja algorytmu z charged PSO również nie przyniosła poprawy wyników, lecz w tym przypadku nie wydaje się to być wynikiem popełnionych przez nas błędów, a po prostu specyfiką tej wersji algorytmu.

Pomimo zgodnego z oczekiwaniami działania klastra umożliwiającego wykorzystanie dostępnej mocy obliczeniowej, również ta część systemu nadaje się do usprawnienia. Należałoby sprawdzić, czy zmiana topologii klastra oraz zastosowanie różnych rodzajów rojów dla różnych węzłów może przynieść poprawę wyników w stosunku do przedstawionej wersji standardowej.
...
\textbf{Paweł Kupidura} - Odpowiadał za implementację algorytmu w technologii CUDA oraz komunikację pomiędzy CPU a GPU wraz z ich przetestowaniem i opisaniem w pracy. Dodatkowo stworzył pierwotną implementacją algorytmu PSO w C/C++.\\

\textbf{Hubert Rosiak} - Odpowiadał za stworzenie wrappera do platformy COCO. Brał udział w tworzeniu struktury węzła obliczeniowego. Zajmował się przeglądem i opisem literatury, znanych rezultatów i zastosowań algorytmu PSO. Brał udział w opisywaniu części systemu działającej na CPU oraz w planowaniu, przeprowadzaniu oraz dokumentacji i opracowaniu wyników.\\

\textbf{Kacper Trojanowski} - Brał udział w tworzeniu struktury klastra obliczeniowego i odpowiadał za zaprojektowanie komunikacji pomiędzy jego węzłami oraz testowanie awaryjności. Stworzył algorytm PSO w C\# umożliwiający jego łatwą modyfikację i hybrydyzację. Brał udział w opisywaniu części systemu działającej na CPU oraz w planowaniu, przeprowadzaniu oraz dokumentacji i opracowaniu wyników.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Podsumowanie}

\section{Nasz program}
...
Celem niniejszej pracy była implementacja wybranego populacyjnego algorytmu optymalizacji globalnej wraz z wykorzystującym ją systemem do optymalizacji funkcji, który zapewniałby możliwość wykorzystania zarówno obliczeń równoległych jak i rozproszonych w celu poprawy działania.

Jako algorytm stanowiący bazę naszego systemu wybraliśmy PSO, gdyż posiada wymagane przez nas cechy.
	Algorytm PSO naśladuje występujące w przyrodzie, czy też ogólniej - świecie rzeczywistym, zachowania rojów, stad czy społeczeństw i jest algorytmem koncepcyjnie bardzo prostym, a co więcej, prostota ta przekłada się na łatwość implementacji. 
...
\bibitem{topologieRys}http://dev.heuristiclab.com/trac.fcgi/raw-attachment/wiki/Documentation/
Reference/Particle\%20Swarm\%20Optimization/PSO\_Topologies.png
